...
async function addUser(username, password, mail) {
	return new Promise((resolve, reject) => {
		MongoClient.connect(dbString, async (err, client) => {
			...
			const userexisting = await userExists(username);
			if (userexisting) {
				resolve(1); client.close(); return;
			}

			const emailexisting = await emailExists(mail);
			if (emailexisting) {
				resolve(2); client.close(); return;
			}

			bcrypt.genSalt(saltRounds, (err, salt) => {
				bcrypt.hash(password, salt, (err, hash) => {
					const doc = {user: username, pass: hash, email: mail};
